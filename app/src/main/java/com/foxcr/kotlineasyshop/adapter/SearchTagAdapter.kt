...
class SearchTagAdapter constructor(context: Context, var datas:MutableList<SearchArticleResp.DatasBean.TagsBean>): TagAdapter<SearchArticleResp.DatasBean.TagsBean>(datas){
    private val mInflater: LayoutInflater = LayoutInflater.from(context)

    fun setNewData(data: MutableList<SearchArticleResp.DatasBean.TagsBean>?) {
        this.datas = data ?: mutableListOf()
        notifyDataChanged()
    }

    fun addData(data: MutableList<SearchArticleResp.DatasBean.TagsBean>) {
        datas.addAll(data)
        notifyDataChanged()
    }
    ...
    override fun getView(
        parent: FlowLayout,
        position: Int,
        t: SearchArticleResp.DatasBean.TagsBean
    ): View {
